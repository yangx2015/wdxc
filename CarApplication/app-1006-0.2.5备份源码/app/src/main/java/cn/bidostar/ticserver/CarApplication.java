...
import cn.bidostar.ticserver.service.StopPushJobSchedulerService;
...
public class CarApplication extends Application {
    ...
    public void lockWakeup(){
        ...
        if(!wakeLock.isHeld()){
            //持续加锁2个小时。大概一个20M视频，网络正常时，上传约20秒。2个小时正常情况可以上传6个小时的视频
    ...
    public void startStopPushJobService(){
        try{
            JobScheduler scheduler = (JobScheduler) getSystemService(Context.JOB_SCHEDULER_SERVICE);
            JobInfo stopPushJobInfo = new JobInfo.Builder(1, new ComponentName(this, StopPushJobSchedulerService.class))
                    .setMinimumLatency(AlarmManager.INTERVAL_HOUR)
                    //.setRequiredNetworkType(JobInfo.NETWORK_TYPE_NONE)//不管是否有网络这个任务都会被执行
                    .build();

            if (scheduler != null) {
                scheduler.schedule(stopPushJobInfo);
            }
        }catch (Exception e){
