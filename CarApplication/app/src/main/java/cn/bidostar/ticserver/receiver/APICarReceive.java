 ...
 *
 */
public final class APICarReceive extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (mAppContext == null){
            mAppContext = CarApplication.getApplication().getApplicationContext();
        }
        ...
        try{
            if(intent.getAction().equals(CarIntents.ACTION_CAMERA_SNAPSHOT_CALLBACK)) {
                ...
                if(path!=null&&path.trim().length()>1){
                    ServerApiUtils.getInstance().uploadFile("100",path, CAR_HB_MP4_TASKID,ServerApiUtils.fileUploadCallback);//直接上传到服务器 出现这个表示是后台截图的视频，所以直接上传即可
                }
                ...
                if(filename!=null && filename.contains("lock")) {//如果再lock目录，则是碰撞的直接上传
                    ...
                    ServerApiUtils.getInstance().uploadFile("102", filename, CAR_HB_MP4_TASKID, ServerApiUtils.fileUploadCallback);//截图视频，直接上传
                }
