...
public interface ClXlMapper extends Mapper<ClXl> {
    ...
    @Select("SELECT Z.FW AS FW,Z.ID AS ID,Z.MC AS MC,(SELECT COUNT(1) FROM CL_CLYXJL JL WHERE JL.XL_ID = #{id} AND JL.ZD_ID=Z.ID AND TO_CHAR(JL.CJSJ,'yyyy-MM-dd') >= TO_CHAR(sysdate,'yyyy-MM-dd') ) AS CLSL ,Z.JD AS ZD_JD,Z.WD AS ZD_WD FROM CL_XLZD T,CL_ZD Z WHERE T.ZD_ID=Z.ID AND T.ZT='00' AND Z.ZT ='00' AND  T.XL_ID= #{id} ORDER BY T.XH ASC")
    @Results({
