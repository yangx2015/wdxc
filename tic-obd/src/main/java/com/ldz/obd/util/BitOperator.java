...
public class BitOperator {
     ...
     * @return
     */
    public static byte testCheckSum4JT808(byte[] bs, int start, int end) {
        if (start < 0 || end > bs.length){
            throw new ArrayIndexOutOfBoundsException("getCheckSum4JT808 error : index out of bounds(start=" + start + ",end=" + end + ",bytes length=" + bs.length + ")");
        }
        byte cs = bs[start];
...
//        byte[] by={0x10,0x16,0x01,0x19,0x02,0x28,0x20,(byte) 0x84,0x00,0x40,0x01,0x00,0x0D,0x20,0x04,0x18,0x17,0x16,0x39,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x06,0x00,0x00,0x0A,0x0A,0x00,0x00,0x00,0x4B,0x00,0x20,0x01,0x00,0x1C,0x7F,0x1F,0x38,0x31,0x00,0x30,0x7D,0x52,0x05,0x14,(byte) 0xFF,0x01,0x00,0x02,0x11,0x01,0x00,0x2B,(byte) 0x80,0x03,0x00,0x00,(byte) 0xB3,(byte) 0xB1,0x1B,(byte) 0xDF,0x77, (byte) 0xD0,0x00,0x00,0x00,0x05,0x01};
        //          00 00 00 00 00 00 20 03 00 03 01 00 05
        byte[] by1={0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x20 ,0x03 ,0x00 ,0x01 ,0x00};
//        int bybyte=testCheckSum4JT808(by,0,by.length);
        byte bybyte1=testCheckSum4JT808(by1,0,by1.length);
