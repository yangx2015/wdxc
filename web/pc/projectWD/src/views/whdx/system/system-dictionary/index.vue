...
<script>
	...
	export default {
		...
		methods: {
			...
			removeDc(item,index){
				var v = this
				this.$http.post(configApi.DICTIONARY.DELE,{'ids':[item.lmdm]}).then((res) =>{
					console.log('字典数据',res)
					if(res.code===200){
			...
			findDicList(item,index,mess){
				var v = this
				this.$http.post(configApi.DICTIONARY_LIST.QUERY,{'zdlmdm':item.lmdm,'zdmcLike':mess}).then((res) =>{
					console.log('字典数据',res)
					if(res.code===200){
			...
			removeDcList(item,items){
				var v = this
				this.$http.post(configApi.DICTIONARY_LIST.DELE,{'ids':[items.zdId]}).then((res) =>{
					console.log('字典数据',res)
					if(res.code===200){
						v.$Message.success('操作成功');
					}else{
