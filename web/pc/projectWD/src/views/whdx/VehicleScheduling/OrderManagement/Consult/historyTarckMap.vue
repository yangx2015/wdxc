...
<script>
	...
	export default {
		...
		methods: {
            ...
            init(){
                this.gpsList = this.$parent.detail.gpsLog;
			...
			line(){
				...
				// 增加起点
                var pt1 = new BMap.Point(v.gpsList[0].bdjd, v.gpsList[0].bdwd);
				...
				// 增加终点
                var pt2 = new BMap.Point(v.gpsList[v.gpsList.length-1].bdjd, v.gpsList[v.gpsList.length-1].bdwd);
			...
			animationDot(){
                ...
                if (this.movingMarker != null){
                    ...
                    if (this.playAndStopBtnGroup.playIndex == this.gpsList.length){
                        //循环执行完成后，重置播放轨迹点
					...
					try{
                        //取出下一个动画节点
                        var moveData = this.gpsList[this.playAndStopBtnGroup.playIndex];
                        //更新地图移动轨迹
